...
// Iterators
    ...
    typedef cb_details::iterator< circular_buffer<T, Alloc>, cb_details::const_traits<std::allocator_traits<Alloc> > > const_iterator;

    //! A (random access) iterator used to iterate through the <code>circular_buffer</code>.
    typedef cb_details::iterator< circular_buffer<T, Alloc>, cb_details::nonconst_traits<std::allocator_traits<Alloc> > > iterator;
            ...
            for (pointer first = m_first; dest < src; src = first) {
                for (size_type ii = 0; src < m_end; ++src, ++dest, ++moved, ++ii) {
                    ...
                    if (is_uninitialized(dest)) {
                        std::allocator_traits<Alloc>::construct(m_alloc, std::addressof(*dest), std::move_if_noexcept(*src));
                        ++constructed;
                    } else {
...
// Size and capacity
    ...
    */
    size_type max_size() const noexcept {
        return (std::min<size_type>)(std::allocator_traits<Alloc>::max_size(m_alloc), (std::numeric_limits<difference_type>::max)());
    }
    ...
    void push_front_impl(ValT item) {
        { try {
            if (full()) {
                ...
                decrement(m_first);
                std::allocator_traits<Alloc>::construct(m_alloc, std::addressof(*m_first), static_cast<ValT>(item));
                ++m_size;
            }
        } catch(...) {
            increment(m_first);
...
// Helper methods
    ...
    */
    ...
    //! Specialized initialize method.
    template <class IntegralType>
    void initialize(capacity_type buffer_capacity, IntegralType n, IntegralType item, const std::true_type&) {
        assert(buffer_capacity >= static_cast<size_type>(n)); // check for capacity lower than n
        m_size = static_cast<size_type>(n);
        initialize_buffer(buffer_capacity, item);
        m_first = m_buff;
    ...
    void initialize(capacity_type buffer_capacity,
        ...
        while (first != last && !full()) {
            std::allocator_traits<Alloc>::construct(m_alloc, std::addressof(*m_last), *first++);
            increment(m_last);
            ++m_size;
        }
        while (first != last) {
    ...
    void assign(InputIterator first, InputIterator last, const std::input_iterator_tag&) {
        ...
        assign_n(distance, distance,
            cb_details::make_assign_range
                (std::make_move_iterator(tmp.begin()), std::make_move_iterator(tmp.end()), m_alloc));
    }
    ...
    //! Specialized assign method.
    template <class IntegralType>
    void assign(capacity_type new_capacity, IntegralType n, IntegralType item, const std::true_type&) {
        ...
        if (pos.m_it == 0) {
            ...
            { try {
                for (; ii < construct; ++ii, increment(p))
                    std::allocator_traits<Alloc>::construct(m_alloc, std::addressof(*p), *wrapper());
