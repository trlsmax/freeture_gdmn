...
void LocalEvent::computeMassCenter() 
{
    ...
    for (auto& pnt : mAbsPos) {
        x += pnt.x;
        y += pnt.y;
    }

    x /= static_cast<float>(mAbsPos.size());
    y /= static_cast<float>(mAbsPos.size());
...
bool LocalEvent::localEventIsValid() 
{
    ...
    if(!mPosPos.empty()) {
        ...
        for (auto& pnt : mPosPos) {
            xPos += pnt.x;
            yPos += pnt.y;
        }

        xPos /= static_cast<float>(mPosPos.size());
        yPos /= static_cast<float>(mPosPos.size());
        mPosMassCenter = Point(xPos, yPos);

        // Search radius.
        for (auto& pnt : mPosPos) {
            float radius = sqrt(pow((pnt.x - mPosMassCenter.x), 2) + pow((pnt.y - mPosMassCenter.y), 2));
    ...
    if(!mNegPos.empty()) {
        ...
        for(auto& pnt : mNegPos){
            xNeg += pnt.x;
            yNeg += pnt.y;
        }

        xNeg /= static_cast<float>(mNegPos.size());
        yNeg /= static_cast<float>(mNegPos.size());
        mNegMassCenter = Point(xNeg, yNeg);

        // Search radius.
        for(auto& pnt : mNegPos){
            float radius = sqrt(pow((pnt.x - mNegMassCenter.x), 2) + pow((pnt.y - mNegMassCenter.y), 2));
    ...
    if(negCluster && posCluster) {
        ...
        if(!res) {
            ...
            if( surfaceCircle1 != 0 && intersectedSurface != 0 && surfaceCircle2 != 0 ) {
...
Mat LocalEvent::createPosNegAbsMap() 
{
    ...
    for(auto& pnt : mAbsPos){
        map.at<Vec3b>(pnt.y, pnt.x)[0] = 255;
        map.at<Vec3b>(pnt.y, pnt.x)[1] = 255;
        map.at<Vec3b>(pnt.y, pnt.x)[2] = 255;
    }
    ...
    if(!mPosPos.empty()) {
        ...
        for(auto& pnt : mPosPos){
            map.at<Vec3b>(pnt.y, pnt.x)[0] = 0;
            map.at<Vec3b>(pnt.y, pnt.x)[1] = 255;
            map.at<Vec3b>(pnt.y, pnt.x)[2] = 0;
    ...
    if(!mNegPos.empty()) {
        ...
        for(auto& pnt : mNegPos){
            if(map.at<Vec3b>(pnt.y, pnt.x) == Vec3b(0,255,0)) {
                map.at<Vec3b>(pnt.y, pnt.x)[0] = 255;
                map.at<Vec3b>(pnt.y, pnt.x)[1] = 0;
                map.at<Vec3b>(pnt.y, pnt.x)[2] = 0;
            }else {
                map.at<Vec3b>(pnt.y, pnt.x)[0] = 0;
                map.at<Vec3b>(pnt.y, pnt.x)[1] = 0;
