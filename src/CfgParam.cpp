...
void CfgParam::loadDataParam()
{
    ...
    if (!cfg.Get("DATA_PATH", param.data.DATA_PATH)) {
        ...
        fs::path p(param.data.DATA_PATH);
    ...
    if (!param.camInput.ephem.EPHEMERIS_ENABLED) {
        ...
        if (!cfg.Get("SUNRISE_TIME", sunrise_time)) {
            ...
            if (sunrise_time.find(":") != std::string::npos) {
                ...
                std::vector<std::string> tokens = TimeDate::split(sunrise_time.c_str(), ':');
                param.camInput.ephem.SUNRISE_TIME.clear();
                for (std::string tok : tokens) {
                    param.camInput.ephem.SUNRISE_TIME.push_back(atoi(tok.c_str()));
                }
        ...
        if (!cfg.Get("SUNSET_TIME", sunset_time)) {
            ...
            if (sunset_time.find(":") != std::string::npos) {
                std::vector<std::string> tokens = TimeDate::split(sunset_time.c_str(), ':');
                param.camInput.ephem.SUNSET_TIME.clear();
                for (std::string tok : tokens)
                    param.camInput.ephem.SUNSET_TIME.push_back(atoi(tok.c_str()));
    ...
    if (!cfg.Get("ACQ_REGULAR_ENABLED", param.camInput.regcap.ACQ_REGULAR_ENABLED)) {
        ...
        if (param.camInput.regcap.ACQ_REGULAR_ENABLED) {
...

            {
                ...
                if (!cfg.Get("ACQ_REGULAR_OUTPUT", img_output)) {
                    ...
                    try {
                        ...
                        param.camInput.regcap.ACQ_REGULAR_OUTPUT = imgOutput.parseEnum("ACQ_REGULAR_OUTPUT",
                                                                                       img_output);
                    } catch (std::exception &ex) {
                        e = true;
            ...
            if (!cfg.Get("ACQ_REGULAR_CFG", regAcqParam)) {
                ...
                std::string sep("HMSEGFN");
                std::vector<std::string> res1;
                res1.push_back(regAcqParam);
                for (size_t i = 0; i < sep.size(); i++) {
                    std::vector<std::string> result;
                    for (std::string str : res1) {
                        std::vector<std::string> tmp = TimeDate::split(str.c_str(), sep[i]);
                ...
                if (res1.size() == 7) {
                    ...
                    if (atoi(res1.at(0).c_str()) < 0 || atoi(res1.at(0).c_str()) >= 24) {
                        e = true;
                        param.camInput.errormsg.push_back(
                                "- ACQ_REGULAR_CFG : Hours can't have the value <" + res1.at(0) +
                    ...
                    if (atoi(res1.at(1).c_str()) < 0 || atoi(res1.at(1).c_str()) >= 60) {
                        e = true;
                        param.camInput.errormsg.push_back(
                                "- ACQ_REGULAR_CFG : Minutes can't have the value <" + res1.at(1) +
                                ">.\nAvailable range is from 0 to 23.");
                    }
                    ...
                    if (atoi(res1.at(2).c_str()) < 0 || atoi(res1.at(2).c_str()) >= 60) {
                        e = true;
                        param.camInput.errormsg.push_back(
                                "- ACQ_REGULAR_CFG : Seconds can't have the value <" + res1.at(2) +
                                ">.\nAvailable range is from 0 to 23.");
                    }

                    param.camInput.regcap.ACQ_REGULAR_CFG.interval =
                            atoi(res1.at(0).c_str()) * 3600 + atoi(res1.at(1).c_str()) * 60 + atoi(res1.at(2).c_str());
                    ...
                    if (mine != -1 && maxe != -1) {
                        if (param.camInput.regcap.ACQ_REGULAR_CFG.exp < mine ||
                            param.camInput.regcap.ACQ_REGULAR_CFG.exp > maxe) {
                            param.camInput.errormsg.push_back("- ACQ_REGULAR_CFG : Exposure value <" +
                                                              Conversion::intToString(
                                                                      param.camInput.regcap.ACQ_REGULAR_CFG.exp) +
                                                              "> is not correct. \nAvailable range is from " +
                                                              Conversion::intToString(mine) + " to " +
                                                              Conversion::intToString(maxe));
                            //e = true;
                    ...
                    if (ming != -1 && maxg != -1) {
                        if (param.camInput.regcap.ACQ_REGULAR_CFG.gain < ming ||
                            param.camInput.regcap.ACQ_REGULAR_CFG.gain > maxg) {
                            param.camInput.errormsg.push_back("- ACQ_REGULAR_CFG : Gain value <" +
                                                              Conversion::intToString(
                                                                      param.camInput.regcap.ACQ_REGULAR_CFG.gain) +
                                                              "> is not correct. \nAvailable range is from " +
                                                              Conversion::intToString(ming) + " to " +
                                                              Conversion::intToString(maxg));
                            e = true;
                    ...
                    if (fmt.getStringEnum(param.camInput.regcap.ACQ_REGULAR_CFG.fmt) == "") {
                        e = true;
                        param.camInput.errormsg.push_back(
                                "- ACQ_REGULAR_CFG : Fail to extract pixel format on " + regAcqParam +
                                ". Check if index <" + res1.at(5) + "> exits.");
                    }
                }
            }
        }
            ...
            if (!param.camInput.regcap.ACQ_REGULAR_ENABLED) {
...

                {
                    ...
                    if (!cfg.Get("ACQ_SCHEDULE_OUTPUT", img_output)) {
                        ...
                        try {
                            EParser<ImgFormat> imgOutput;
                            param.camInput.schcap.ACQ_SCHEDULE_OUTPUT = imgOutput.parseEnum("ACQ_SCHEDULE_OUTPUT",
                                                                                            img_output);
                        } catch (std::exception &ex) {
                            e = true;
...

                {
                    ...
                    if (!cfg.Get("ACQ_SCHEDULE", sACQ_SCHEDULE)) {
                        ...
                        vector<string> tokens = TimeDate::split(sACQ_SCHEDULE.c_str(), ',');
                        for (string s : tokens) {
                        ...
                        for (int i = 0; i < sch1.size(); i++) {
                            ...
                            result.push_back(sch1.at(i));
                            for (size_t i = 0; i < sep.size(); i++) {
                                vector<string> tmp;
                                for (size_t j = 0; j < result.size(); j++) {
                                    vector<string> ss = TimeDate::split(result[j].c_str(), sep[i]);
                            ...
                            if (sp.size() == 7) {
                                ...
                                if (spa.hours < 0 || spa.hours >= 24) {
                                    ...
                                    status = false;
                                    param.camInput.errormsg.push_back(
                                            "- ACQ_SCHEDULE : In " + sch1.at(i) + ". Hours can't have the value <" +
                                            Conversion::intToString(spa.hours) +
                                            ">.\nAvailable range is from 0 to 23.");
                                }
                                ...
                                if (spa.min < 0 || spa.min >= 60) {
                                    ...
                                    status = false;
                                    param.camInput.errormsg.push_back(
                                            "- ACQ_SCHEDULE : In " + sch1.at(i) + ". Minutes can't have the value <" +
                                            Conversion::intToString(spa.min) + ">.\nAvailable range is from 0 to 59.");
                                }
                                ...
                                if (spa.sec < 0 || spa.sec >= 60) {
                                    ...
                                    status = false;
                                    param.camInput.errormsg.push_back(
                                            "- ACQ_SCHEDULE : In " + sch1.at(i) + ". Seconds can't have the value <" +
                                            Conversion::intToString(spa.sec) + ">.\nAvailable range is from 0 to 59.");
                                }
                                ...
                                if (mine != -1 && maxe != -1) {
                                    if (spa.exp < mine || spa.exp > maxe) {
                                        param.camInput.errormsg.push_back(
                                                "- ACQ_SCHEDULE : In " + sch1.at(i) + ". Exposure value <" +
                                ...
                                if (ming != -1 && maxg != -1) {
                                    if (spa.gain < ming || spa.gain > maxg) {
                                        param.camInput.errormsg.push_back(
                                                "- ACQ_SCHEDULE : In " + sch1.at(i) + ". Gain value <" +
                                ...
                                if (spa.rep < 0 || spa.rep >= 60) {
                                    ...
                                    status = false;
                                    param.camInput.errormsg.push_back(
                                            "- ACQ_SCHEDULE : One repetition must be defined at least.");
                                }
                                ...
                                if (fmt.getStringEnum(spa.fmt) == "") {
                                    ...
                                    status = false;
                                    param.camInput.errormsg.push_back(
                                            "- ACQ_SCHEDULE : Fail to extract pixel format for :  " + sch1.at(i) +
                                            ". Index <" + sp.at(5) + "> not exist.");
                                }
                        ...
                        if (param.camInput.schcap.ACQ_SCHEDULE.size() != 0) {
                            ...
                            do {
                                ...
                                for (it = param.camInput.schcap.ACQ_SCHEDULE.begin();
                                     it != param.camInput.schcap.ACQ_SCHEDULE.end(); ++it) {
                    ...
                    if (param.DEVICE_ID.first.second) {
                        ...
                        switch (inputType) {
                                ...
                                if (vidParamIsCorrect()) {
                                    ...
                                    for (int i = 0; i < param.vidInput.INPUT_VIDEO_PATH.size(); i++) {
                                        ...
                                        if (cap.isOpened()) {
                                            ...
                                            if (cap.get(CAP_PROP_FRAME_WIDTH) != tempmask.cols) {
                                                e = true;
                                            ...
                                            e = true;
                                            param.det.errormsg.push_back(
                                                    "- ACQ_MASK_PATH : Check mask's size. Fail to open " +
                                                    param.vidInput.INPUT_VIDEO_PATH.at(i));
                                        }
                                    }
                            ...
                            case SINGLE_FITS_FRAME : {
                                if (framesParamIsCorrect()) {
                                    for (int i = 0; i < param.framesInput.INPUT_FRAMES_DIRECTORY_PATH.size(); i++) {
                                        ...
                                        string filefound = "";
                                        ghc::filesystem::path p(param.framesInput.INPUT_FRAMES_DIRECTORY_PATH.at(i));
                                        for (ghc::filesystem::directory_iterator file(p); file != ghc::filesystem::directory_iterator(); ++file) {
                                            ghc::filesystem::path curr(file->path());
                                        ...
                                        if (fitsfilefound) {
                                            Fits2D f(filefound, logger);
                                            int h = 0, w = 0;

                                            if (!f.readIntKeyword("NAXIS1", w)) {
                                                e = true;
                                                param.det.errormsg.push_back(
                                                        "- ACQ_MASK_PATH : Check mask's size. Fail to read NAXIS1. " +
                                                        param.framesInput.INPUT_FRAMES_DIRECTORY_PATH.at(i));
                                            }

                                            if (!f.readIntKeyword("NAXIS2", h)) {
                                                e = true;
                                                param.det.errormsg.push_back(
                                                        "- ACQ_MASK_PATH : Check mask's size. Fail to read NAXIS2. " +
                                                        param.framesInput.INPUT_FRAMES_DIRECTORY_PATH.at(i));
                                            }
                                                ...
                                                if (h != tempmask.rows) {
                                                    ...
                                                    param.det.errormsg.push_back("- ACQ_MASK_PATH : Mask's height (" +
                                                                                 Conversion::intToString(
                                                                                         tempmask.rows) +
                                                                                 ") is not correct with " +
                                                                                 param.framesInput.INPUT_FRAMES_DIRECTORY_PATH.at(
                                                                                         i) + " (" +
                                            ...
                                            e = true;
                                            param.det.errormsg.push_back(
                                                    "- ACQ_MASK_PATH : Check mask's size. No fits file found in " +
                                                    param.framesInput.INPUT_FRAMES_DIRECTORY_PATH.at(i));
                                        }
                                    }
                                }
                            ...
                            default :
                                e = true;
                                param.det.errormsg.push_back(
                                        "- ACQ_MASK_PATH : Fail to create device to check mask's size.");
                        ...
                        e = true;
                        param.det.errormsg.push_back(
                                "- ACQ_MASK_PATH : Fail to create device to check mask's size. CAMERA_ID not loaded.");
                    }
                }
            }
        }
    ...
    if (!cfg.Get("DET_DEBUG", param.det.DET_DEBUG)) {
        ...
        if (param.det.DET_DEBUG) {
            ...
            if (!cfg.Get("DET_DEBUG_PATH", param.det.DET_DEBUG_PATH)) {
                ...
                namespace fs = ghc::filesystem;
                fs::path p(param.det.DET_DEBUG_PATH);
            ...
            if (!cfg.Get("DET_DEBUG_PATH", param.det.DET_DEBUG_PATH)) {
                ...
                if (!fs::exists(p)) {
                    try {
                        ...
                        param.det.errormsg.push_back(
                                "- DET_DEBUG_PATH : Can't create Debug Path. Debug Path must exist as DET_DEBUG_UPDATE_MASK is enabled.");
                        e = true;
                    }
                }
            }
    ...
    if (!cfg.Get("MAIL_DETECTION_ENABLED", param.mail.MAIL_DETECTION_ENABLED)) {
        ...
        if (param.mail.MAIL_DETECTION_ENABLED) {
            ...
            if (!cfg.Get("MAIL_RECIPIENT", mailRecipients)) {
                ...
                vector<string> tokens = TimeDate::split(mailRecipients.c_str(), ',');
                for (string tok : tokens) {
                    param.mail.MAIL_RECIPIENTS.push_back(tok);
                }
            }
            ...
            if (!cfg.Get("MAIL_CONNECTION_TYPE", smtp_connection_type)) {
                ...
                try {
                    ...
                    param.mail.MAIL_CONNECTION_TYPE = smtp_security.parseEnum("MAIL_CONNECTION_TYPE",
