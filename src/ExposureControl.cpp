...
bool ExposureControl::controlExposureTime(Device *camera, Mat image, TimeDate::Date imageDate, Mat mask,
                                          double minExposureTime, double fps)
{
        ...
        if (mSecTime > autoExposureTimeInterval) {
            ...
            if (autoExposureFinished) {
                ...
                if (frameSkippedCounter <= frameToSkip) {
                    ...
                    logger->info("{} -> EXP : {} MSV : {}", mNbFramesControlled, exposureValue, msv);
                    ...
                    if (!autoExposureInitialized) {
                        if (autoExposureSaveImage) {
                            ...
                            putText(saveFrame, "BEFORE EC -> MSV : " + Conversion::floatToString(msv), cvPoint(20, 20),
                                    FONT_HERSHEY_COMPLEX_SMALL, 0.8, cvScalar(255), 1, CV_AA);
                            putText(saveFrame, "EXP : ? " /*Conversion::intToString(camera->mCam->getExposureTime())*/,
                                    cvPoint(20, 40), FONT_HERSHEY_COMPLEX_SMALL, 0.8, cvScalar(255), 1, CV_AA);
                            if (checkDataLocation(imageDate))
                                SaveImg::saveBMP(saveFrame, finalDataLocation + "expControl_" +
                                                            TimeDate::getYYYYMMDDThhmmss(imageDate) + "_before");
                        }

                        // Get minimum exposure time.
                        minCameraExposureValue = minExposureTime;
                        logger->info("Min EXP : {}", minCameraExposureValue);
                        ...
                        logger->info("Max EXP : {}", maxCameraExposureValue);
                        ...
                        logger->info("Set EXP to : {}", minCameraExposureValue);
                        ...
                        if (step1) {
                            ...
                            if (incrementExposureTimeValue) {
                                double delta = 1000;
                                logger->info("STEP 1 : Incrementation by {}", delta);
                                ...
                                logger->info("Set EXP to : {}", exposureValue);
                            } else {
                                logger->info("STEP 1 : Analyse msv");
                                logger->info("MSV ARRAY1 SIZE : {}EXP ARRAY1 SIZE {}",
                                             msvArray_1.size(),
                                             expArray_1.size());
                                    ...
                                    for (int i = 0; i < msvArray_1.size(); i++) {
                                        if ((i + 1) < msvArray_1.size()) {
                                            ...
                                            if (msvArray_1.at(i) < 2.5 && msvArray_1.at(i + 1) > 2.5) {
                                                ...
                                                logger->info("New interval found -> MSV[{}-{}] EXP[{}-{}]",
                                                        msvMin_1,
                                                        msvMax_1,
                                                        expMin_1,
                                                        expMax_1);
                            ...
                            if (incrementExposureTimeValue) {
                                double delta = 30;
                                logger->info("STEP 2 : Incrementation by {}", delta);
                                ...
                                logger->info("STEP 2 : Analyse msv");
                                logger->info("MSV ARRAY2 SIZE : {}EXP ARRAY2 SIZE {}",
                                             msvArray_2.size(),
                                             expArray_2.size());
                                    ...
                                    for (int i = 0; i < msvArray_2.size(); i++) {
                                        if ((i + 1) < msvArray_2.size()) {
                                            if (msvArray_2.at(i) < 2.5 && msvArray_2.at(i + 1) > 2.5) {
                                                ...
                                                logger->info("Set EXP to : {}", exposureValue);
                                                logger->info("Interval choose -> MSV[{}-{}] EXP[{}-{}]",
                                                        msvMin_2,
                                                        msvMax_2,
                                                        expMin_2,
                                                        expMax_2);
                                ...
                                logger->info("FINAL EXPOSURE : {}", finalExposureTime);
                            }
                        } else {
                            ...
                            if (autoExposureSaveImage) {
                                ...
                                image.copyTo(saveFrame);
                                if (saveFrame.type() == CV_16U) saveFrame = Conversion::convertTo8UC1(saveFrame);
                                putText(saveFrame, "AFTER EC -> MSV : " + Conversion::floatToString(msv),
                                        cvPoint(20, 20), FONT_HERSHEY_COMPLEX_SMALL, 0.8, cvScalar(255), 1, CV_AA);
                                putText(saveFrame, "EXP : " + Conversion::intToString(finalExposureTime),
                                        cvPoint(20, 40), FONT_HERSHEY_COMPLEX_SMALL, 0.8, cvScalar(255), 1, CV_AA);

                                if (checkDataLocation(imageDate))
                                    SaveImg::saveBMP(saveFrame, finalDataLocation + "expControl_" +
                                                                TimeDate::getYYYYMMDDThhmmss(imageDate) + "_after");
                            }
                            ...
                            if (autoExposureSaveInfos) {
                                if (checkDataLocation(imageDate)) {
                                    ...
                                    infFile << "# DATE : " << d << "  EXP : "
                                            << Conversion::doubleToString(exposureValue) << "  MSV : "
                                            << Conversion::floatToString(msv) << "\n";
