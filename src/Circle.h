    ...
    public :
        ...
        bool computeDiskSurfaceIntersection(    Circle c2,
            ...
            if(distPcNc > c2.getRadius() + mRadius) {
                ...
                if(enableDebug) putText(map, "No intersections." , cv::Point(15,15), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cv::Scalar(0,0,255), 1, CV_AA);
                res = false;

            // Circles coincide.
            }else if(distPcNc == 0 && c2.getRadius() == mRadius) {

                if(enableDebug) putText(map, "Circles coincides." , cv::Point(15,15), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cv::Scalar(0,0,255), 1, CV_AA);
                res = true;

            // A circle is contained inside the other.
            }else if(distPcNc < abs(c2.getRadius() - mRadius)) {

                if(enableDebug) putText(map, "A circle is contained whithin the other." , cv::Point(15,15), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cv::Scalar(0,0,255), 1, CV_AA);
                res = true;
                ...
                if(mPos.y != c2.getCenter().y) {
                    ...
                    if(delta > 0) {
                        ...
                        if(enableDebug) line(map, cv::Point(resX1,resY1 ), cv::Point(resX2,resY2 ), cv::Scalar(255,255,255), 1, CV_AA);
            ...
            if(enableDebug && displayIntersectedSurface) {
                ...
                putText(map, "Intersected surface : " , cv::Point(15,15), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cv::Scalar(0,0,255), 1, CV_AA);
                string msg1 = "- Green circle : " + Conversion::floatToString((intersectedSurface * 100) / surfaceCircle1) + "%" ;
                putText(map, msg1 , cv::Point(15,30), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cv::Scalar(0,0,255), 1, CV_AA);
                string msg2 = "- Red circle : " + Conversion::floatToString((intersectedSurface * 100) / surfaceCircle2)+ "%";
                putText(map, msg2 , cv::Point(15,45), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cv::Scalar(0,0,255), 1, CV_AA);
