        ...
        switch (frmsize.type) {
            case V4L2_FRMSIZE_TYPE_DISCRETE :
                if (chooseHeight == 0 && chooseWidth == 0) {
                    ...
                    if ((abs(mWidth - chooseWidth) > abs(mWidth - frmsize.discrete.width)) &&
                        (abs(mHeight - chooseHeight) > abs(mHeight - frmsize.discrete.height))) {
                        chooseWidth = frmsize.discrete.width;
                        chooseHeight = frmsize.discrete.height;
                    }
                }
    ...
    if (buffer != NULL) {
        switch (mFormat.fmt.pix.pixelformat) {
            ...
            case V4L2_PIX_FMT_YUYV : {
                unsigned char *bigbuffer = (unsigned char *) malloc(
                        mFormat.fmt.pix.height * mFormat.fmt.pix.width * 3 * sizeof(char));
                Mat dispimg(mFormat.fmt.pix.height, mFormat.fmt.pix.width, CV_8UC3, bigbuffer);
                PixFmtConv::YUYV_to_BGR24(buffer, bigbuffer, mFormat.fmt.pix.width, mFormat.fmt.pix.height,
                                          mFormat.fmt.pix.bytesperline);
                cvtColor(dispimg, image, COLOR_BGR2GRAY);
                res = true;
                free(bigbuffer);
            }
                break;
            case V4L2_PIX_FMT_UYVY : {
                ...
                PixFmtConv::UYVY_to_BGR24(buffer, bigbuffer, mFormat.fmt.pix.width, mFormat.fmt.pix.height,
                                          mFormat.fmt.pix.bytesperline);
                Mat dispimg(mFormat.fmt.pix.height, mFormat.fmt.pix.width, CV_8UC3, bigbuffer);
                cvtColor(dispimg, image, COLOR_BGR2GRAY);
                res = true;
...
bool CameraV4l2::getFrameSizeEnum()
{
    ...
    while (ioctl(fd, VIDIOC_ENUM_FRAMESIZES, &frmsize) == 0) {
        switch (frmsize.type) {
            case V4L2_FRMSIZE_TYPE_DISCRETE :
                cout << "[" << frmsize.index << "] : " << frmsize.discrete.width << "x" << frmsize.discrete.height
                     << endl;
                res = true;
                break;
            case V4L2_FRMSIZE_TYPE_CONTINUOUS :
...
bool CameraV4l2::setPixelFormat(CamPixFmt depth)
{
    ...
    if (!fmtFound) {
        auto logger = spdlog::get("acq_logger");
        logger->critical(">> FORMAT {} NOT SUPPORTED !", fstring);
        return false;
    }
